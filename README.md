...
If are working with [encrypted forms](http://docs.opendatakit.org/encrypted-forms), you will need to install the Java Cryptography Extension (JCE) Unlimited Strength Jurisdiction Policy Files or you'll get a `InvalidKeyException`. Download the policy files for Java 8 [here](http://www.oracle.com/technetwork/java/javase/downloads/jce8-download-2133166.html) and for Java 7 [here](www.oracle.com/technetwork/java/javase/downloads/jce-7-download-432124.html).

Note: There might be some compile errors in the IDE about a missing class `BuildConfig`. That class is generated by Android Tools at runtime. The warnings can be ignored.
