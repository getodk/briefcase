...
import static org.opendatakit.briefcase.reused.model.transfer.AggregateServer.clearStoredPrefs;
import static org.opendatakit.briefcase.reused.model.transfer.AggregateServer.isPrefKey;
import static org.opendatakit.briefcase.reused.model.transfer.AggregateServer.normal;
import static org.opendatakit.briefcase.reused.model.transfer.AggregateServer.readFromPrefs;
...
public class AggregateServerTest {
  ...
  public void stores_and_reads_form_pull_sources_using_prefs() {
    ...
    FormMetadata aForm = FormMetadataHelpers.buildFormMetadata(1);
    noCredsServer.storeInPrefs(prefs, true, aForm.getKey().getId());

    AggregateServer credsServer = new AggregateServer(url("http://fizz.baz"), Optional.of(TEST_CREDENTIALS));
    FormMetadata anotherForm = FormMetadataHelpers.buildFormMetadata(2);
    credsServer.storeInPrefs(prefs, true, anotherForm.getKey().getId());

    assertThat(prefs.keys(), hasSize(6));
    assertThat(readFromPrefs(prefs, pullPanelPrefs, aForm.getKey()), isPresentAndIs(noCredsServer));
    assertThat(readFromPrefs(prefs, pullPanelPrefs, anotherForm.getKey()), isPresentAndIs(credsServer));
  }
  ...
  public void can_clear_stored_form_pull_sources() {
    ...
    noCredsServer.clearStoredPrefs(prefs, aForm.getKey().getId());
    credsServer.clearStoredPrefs(prefs, anotherForm.getKey().getId());
  ...
  public void moves_prefs_to_new_storage_when_reading_from_legacy_storage() {
    ...
    readFromPrefs(prefs, pullPanelPrefs, aForm.getKey());
    readFromPrefs(prefs, pullPanelPrefs, anotherForm.getKey());
