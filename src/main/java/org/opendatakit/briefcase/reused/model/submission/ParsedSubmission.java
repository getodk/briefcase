...
public class ParsedSubmission {
  ...
  private ParsedSubmission(SubmissionMetadata submissionMetadata, XmlElement root, ValidationStatus validationStatus, Optional<CipherFactory> cipherFactory, Optional<byte[]> signature) {
  ...
  public static ParsedSubmission plain(SubmissionMetadata submissionMetadata, XmlElement root) {
    return new ParsedSubmission(submissionMetadata, root, NOT_VALIDATED, Optional.empty(), Optional.empty());
  }
  ...
  static ParsedSubmission unencrypted(SubmissionMetadata submissionMetadata, XmlElement root, CipherFactory cipherFactory, byte[] signature) {
    return new ParsedSubmission(submissionMetadata, root, NOT_VALIDATED, Optional.of(cipherFactory), Optional.of(signature));
  }
  ...
  String buildSignature(ParsedSubmission originalSubmission) {
  ...
  ParsedSubmission withPathAndDocument(Path path, Document document) {
    return new ParsedSubmission(submissionMetadata.withSubmissionFile(path), XmlElement.of(document), validationStatus, cipherFactory, signature);
  }

  ParsedSubmission withValidationStatus(ValidationStatus validationStatus) {
    return new ParsedSubmission(submissionMetadata, root, validationStatus, cipherFactory, signature);
  }
