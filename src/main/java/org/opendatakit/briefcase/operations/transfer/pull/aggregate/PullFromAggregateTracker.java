...
class PullFromAggregateTracker {
  ...
  void trackStart() {
    ...
    log.info("Pull {} - {}", key.getId(), message);
  ...
  void trackCancellation(String job) {
    ...
    log.warn("Push {} - {}", key.getId(), message);
    notifyTrackingEvent(message);
  }
  ...
  void trackErrorDownloadingForm(Response response) {
    ...
    log.error("Pull {} - {}: HTTP {} {}", key.getId(), message, response.getStatusCode(), response.getStatusPhrase());
  ...
  void trackNoSubmissions() {
    ...
    log.info("Push {} - {}", key.getId(), message);
    notifyTrackingEvent(message);
  }
