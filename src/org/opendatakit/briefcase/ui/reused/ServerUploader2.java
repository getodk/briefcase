...
import org.opendatakit.briefcase.util.DatabaseUtils;
...
public class ServerUploader2 { private static final Logger log = LoggerFactory.getLogger(ServerUploader.class);
  ...
  public static class SubmissionResponseAction implements AggregateUtils2.ResponseAction {
    ...
    @Override
    public void doAction(AggregateUtils2.DocumentFetchResult result) throws MetadataUpdateException {
  ...
  public boolean uploadForm(FormStatus formToTransfer, File briefcaseFormDefFile, File briefcaseFormMediaDir) {
    ...
    URI u;
    try {
      u = AggregateUtils2.testServerConnectionWithHeadRequest(serverInfo, "formUpload");
  ...
  private URI getUploadSubmissionUri(FormStatus formToTransfer) {
    ...
    try {
      ...
      // We know we are talking to the server because the head request
      // succeeded and had a Location header field.
      u = AggregateUtils2.testServerConnectionWithHeadRequest(serverInfo, "submission");
    } catch (TransmissionException e) {
      formToTransfer.setStatusString(e.getMessage(), false);
      EventBus.publish(new FormStatusEvent(formToTransfer));
      return null;
  ...
  public static final void testServerUploadConnection(ServerConnectionInfo serverInfo) throws TransmissionException {
    AggregateUtils2.testServerConnectionWithHeadRequest(serverInfo, "upload"); // for form upload...
  }
