...
public class MainBriefcaseWindow extends WindowAdapter implements UiStateChangeListener {
  ...
  public void setFullUIEnabled(boolean enabled) {
    ...
    settingsPanel.getTxtBriefcaseDir()
        .setText(briefcaseDirectory == null ? "" : briefcaseDirectory + File.separator + StorageLocation.BRIEFCASE_DIR);
  ...
  private void showIntroDialogIfNeeded() {
    if (BriefcasePreferences.appScoped().getBriefcaseDirectoryOrNull() == null) {
      JOptionPane.showMessageDialog(frame, MessageStrings.BRIEFCASE_WELCOME, APP_NAME, JOptionPane.INFORMATION_MESSAGE,
          imageIcon);
    }
  }
  ...
  static Options addOptions() {
    ...
    Option help = Option.builder("h").longOpt(HELP).desc("Print help information (this screen)").build();

    Option version = Option.builder("v").longOpt(VERSION).desc("Print version information").build();
