...
class Model {
  ...
  private Model(TreeElement model) {
    this.model = model;
  }

  /**
   * Factory of {@link Model} instances that takes a {@link BriefcaseFormDefinition}
   * and extracts its submission element {@link TreeElement} instance.
   *
   * @param formDef {@link BriefcaseFormDefinition} instance
   * @return a new {@link Model} instance
   */
  static Model from(FormDefinition formDef) {
    return new Model(formDef.getSubmissionElement());
  }

  /**
   * Method that lets iterate and (flat)map over all the children {@link Model} members of
   * this instance.
   *
