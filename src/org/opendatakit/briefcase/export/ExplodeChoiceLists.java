...
class ExplodeChoiceLists {
  static CsvFieldMapper decorate(CsvFieldMapper mapper) {
    return (localId, workingDir, model, element, exportMedia, exportMediaPath, explodeChoiceLists, configuration) -> {
      List<Pair<String, String>> output = new ArrayList<>();
      output.addAll(mapper.apply(localId, workingDir, model, element, exportMedia, exportMediaPath, explodeChoiceLists, configuration).collect(toList()));
