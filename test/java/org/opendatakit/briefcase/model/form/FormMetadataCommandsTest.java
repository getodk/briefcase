...
public class FormMetadataCommandsTest {
  ...
  public void setUp() {
    ...
    FormMetadata formMetadata = new FormMetadata(key, Paths.get("/some/path/to/the/form"), false, Cursor.empty(), Optional.empty());
  ...
  public void updates_a_form_as_having_been_pulled_adding_the_last_cursor_used_to_pull_it() {
    ...
    formMetadataPort.execute(updateAsPulled(key, cursor));
  ...
  public void updates_a_forms_last_exported_submission_metadata() {
    ...
    formMetadataPort.execute(updateLastExportedSubmission(key, expectedInstanceId, expectedSubmissionDate, expectedExportDate));
    FormMetadata formMetadata = formMetadataPort.fetch(key).get();
