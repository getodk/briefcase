...
import static org.opendatakit.briefcase.ui.export.components.ConfigurationPanelMode.REQUIRE_PULL_TEXT;
import static org.opendatakit.briefcase.ui.export.components.ConfigurationPanelMode.REQUIRE_SAVE_PASSWORDS;
...
public class ConfigurationPanelModeTest extends AssertJSwingJUnitTestCase {
  ...
  public void decorates_fields_related_to_pulling_before_exporting() {
    ...
    assertThat(component.pullBeforeOverrideField(), scenario.overrideFieldVisible ? is(visible()) : is(not(visible())));
    if (scenario.overrideFieldVisible)
      assertThat(component.pullBeforeOverrideField(), scenario.overrideFieldEnabled ? is(enabled()) : is(not(enabled())));
