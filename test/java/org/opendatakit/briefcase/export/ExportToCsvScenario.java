...
import static org.opendatakit.briefcase.reused.UncheckedFiles.readAllLines;
...
class ExportToCsvScenario {
  ...
  ExportToCsvScenario(Path formDir, Path outputDir, FormDefinition formDef, BriefcaseFormDefinition oldFormDef, Optional<String> instanceID) {
  ...
  void assertSameMedia() {
    ...
    List<Path> oldMedia = walk(oldMediaPath).collect(Collectors.toList());
    List<Path> newMedia = walk(newMediaPath).collect(Collectors.toList());
  ...
  private static String readInstanceId(Path path) {
    return readAllLines(path).get(0).split("instanceID=\"")[1].split("\"")[0];
  }
  ...
  private static Path installForm(Path formDir, final String formName) {
    ...
    maybeGetPath(formName + "_submission.xml")
        ...
        .ifPresent(path -> {
          ...
          walk(path.getParent())
              .filter(file -> file.getFileName().toString().startsWith(formName + "_"))
              .filter(file -> !file.getFileName().toString().endsWith(".pem"))
              .filter(file -> !file.getFileName().toString().endsWith(".tpl"))
              .forEach(file -> {
